...
<div>
     <h1 class="w-full text-center text-2xl my-5 font-Roboto text-paleYellow box1 font-semibold">
  POST JOB
</h1> 
  
  
<form action="{{ route('job.store') }}" class="w-full" enctype="multipart/form-data" method="POST">
...
@csrf
     ...
     <div class="md:flex md:items-center mb-6">
      <div class="md:w-1/3">
          <label for="title" class="font-light">
               ...
               <div class="md:w-2/3">
                 <input type="text" id="title" name="title" class="appearance-none border-none rounded w-full py-2 px-4 box bg-paleYellow focus:outline-none " value="{{old('title')}}">
            @error('title')
  ...
  <div class="w-full md:flex md:items-center md:gap-4 mb-6 ">
    ...
    <div class="relative">
     ...
     <div class="md:flex md:items-center mb-6">
      <div class="md:w-1/3">
       ...
       <div class="md:w-2/3">
      <input type="text" id="email" name="email" class="appearance-none border-none box bg-paleYellow     rounded w-full py-2 px-4 focus:outline-none "value="{{old('email')}}">
       @error('email')
    <p class="text-red text-xs mt-1 ">{{ $message }}</p>
    @enderror
          </div>
   ...
   <div class="md:flex md:items-center mb-6">
      <div class="md:w-1/3">
       ...
       <div class="md:w-2/3">
      <input type="text" id="location" name="location" class="appearance-none border-none box bg-paleYellow rounded w-full py-2 px-4 focus:outline-none "value="{{old('location')}}">
           @error('location')
    <p class="text-red text-xs mt-1 ">{{ $message }}</p>
    @enderror
          </div>
      ...
      <div class="md:w-1/3">
          <label for="name" class="font-light">
               ...
               <div class="md:w-2/3">
                 ...
                 focus:outline-none " value="{{old('name')}}">
       @error('name')
    <p class="text-red text-xs mt-1 ">{{ $message }}</p>
    @enderror
               </div>
  ...
  <div class="flex flex-col mb-6">
   ...
   <textarea class='p-5 box bg-paleYellow rounded w-full' name="description" cols="25" rows="10"id="description" >{{old('description')}}</textarea>
  @error('description')
    <p class="text-red text-xs mt-1 ">{{ $message }}</p>
    @enderror
   </div>
